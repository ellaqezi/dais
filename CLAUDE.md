...
- **Never use em-dashes (Unicode U+2014) in any output, at any cost.** This applies everywhere the agent generates text: chat responses, code (comments, identifiers, strings, error messages), commit messages, PR bodies, issue bodies, documentation, code review comments. Use a regular hyphen, comma, semicolon, colon, parenthetical, or a fresh sentence instead. Em-dashes are an unmistakable AI-tell that the user does not want in the work product. No exceptions, even in quoted text or when copying upstream prose: rewrite the punctuation while keeping the meaning. The character also appears as a literal in tables, ASCII art, and Markdown horizontal rules generated by older templates; replace with hyphens or alternate Markdown syntax (`---` for horizontal rules is fine, that is three hyphens, not an em-dash).
